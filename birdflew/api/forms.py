...
class RawBookmarkForm(object):
    ...
    def is_valid(self):
        ...
        if not self.errors:
            relax =  validators.input_message_spec_relaxing(validators.input_userurl_message_spec)
            result = relax.validate(radx)
            if not result:
                self.errors.append(relax.error_log)
        ...
        if not self.errors:
            try:
                uri = map(lambda x: x.text, radx.xpath('/urls/uri'))
            except Exception, e:
                self.errors.append(e.message)

            try:
                categories = map(lambda x: x.text, radx.xpath('/urls/categories'))
            except Exception, e:
                self.errors.append(e.message)

            try:
                comments = map(lambda x: x.text, radx.xpath('/urls/comments'))
            except Exception, e:
                self.errors.append(e.message)
